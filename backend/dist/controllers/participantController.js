...
class ParticipantController {
    // 참가 신청 (공개)
    static async applyParticipant(req, res) {
        try {
            const { error, value } = validation_1.participantSchema.validate(req.body);
            if (error) {
            ...
            // 중복 신청 확인 (전화번호 기준)
            const existingParticipant = await database_1.prisma.participant.findFirst({
                where: {
            ...
            // 전화번호 포맷팅
            value.phone = (0, validation_2.formatVietnamesePhone)(value.phone);
            // 참가자 생성
    ...
    static async getParticipants(req, res) {
        try {
            const { page = '1', limit = '20', status, skillLevel, gender, search } = req.query;
            ...
            const [participants, totalCount] = await Promise.all([
                database_1.prisma.participant.findMany({
                    where,
                    orderBy: { createdAt: 'desc' },
                    skip,
                    take: limitNum,
                ...
                database_1.prisma.participant.count({ where })
            ]);
            // 통계 정보
            const stats = await database_1.prisma.participant.groupBy({
    ...
    static async updateApprovalStatus(req, res) {
        try {
            ...
            const updatedParticipant = await database_1.prisma.participant.update({
                where: { id: participantId },
                data: {
                    approvalStatus,
    ...
    static async updateParticipant(req, res) {
        try {
            ...
            if (filteredData.phone) {
                filteredData.phone = (0, validation_2.formatVietnamesePhone)(filteredData.phone);
            }
            const participant = await database_1.prisma.participant.findUnique({
                where: { id: participantId },
            });
    ...
    static async deleteParticipant(req, res) {
        try {
            ...
            await database_1.prisma.participant.delete({
                where: { id: participantId },
            });
            res.json({
                success: true,
